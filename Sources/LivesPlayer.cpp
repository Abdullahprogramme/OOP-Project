...
LivesPlayer::LivesPlayer(string name, int health, int attack, int num_of_fights, int current_score) {
    this->name = name;
    this->health = health;
    this->attack = attack;
...
int LivesPlayer::move(vector<vector<vector<pair<char, void*>>>>& map, Room* room) {  
    ...
    return coin_score; // Return the score from the coin collected
}
