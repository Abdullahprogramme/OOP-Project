...
void Enemy1::findPlayer(vector<vector<char>> &map, int player_x, int player_y) {
    int rows = map.size();
    int cols = map[0].size();
    ...
    if (nextMove != make_pair(-1, -1)) {
        // 40% chance to make a random move instead of the optimal one
        if (rand() % 100 < 40) {
            for (auto [dx, dy] : directions) {
